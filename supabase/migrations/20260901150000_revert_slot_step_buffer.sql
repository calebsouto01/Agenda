-- Reverts the automatic post-appointment buffer added in
-- 20260901140000_slot_step_as_buffer_after_appointment.sql. The interval
-- after an appointment is now chosen by the administrator at accept time
-- (extending that appointment's ends_at directly), not auto-applied to
-- every appointment via slot_step_minutes. slot_step_minutes goes back to
-- only controlling the spacing between candidate slot times.
